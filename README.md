...
词条提取支持：递归扫描资源目录；从 7z/ZIP/HDF5 容器提取 XML 元数据词条；解析 `.spsm` 智能材质中的图层名；提取 `label`、`text`、`group`、`description`、`category`、`keywords`、`values` 等字段；提取 GLSL 元数据；可选提取普通文件名（默认开）和文件夹名（默认关）。已含中文的原文不会被提取；已有输出中的已有译文会保留，新词条以空译文加入。
