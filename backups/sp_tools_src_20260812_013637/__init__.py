...
def _inside_layers_panel(obj):
    dock = _layers_dock()
    if not _safe(dock):
        return False
    target_window = dock.window()
    current = obj
    while current is not None:
        if current is dock:
            return True
        if current.isWidgetType():
            if current.window() != target_window:
                return False
        current = current.parent()
    return False


def _on_layer_panel_changed():
    """图层面板刷新：只在“切换图层”或“通道集合变化”时重建（事件驱动，不轮询）。"""
    global _LAST_SELECTED_UID, _LAST_PANEL_CHECK_TIME
    # 节流：绘画期间图层面板高频重绘，最多每 100ms 检查一次
    now = time.monotonic()
    if now - _LAST_PANEL_CHECK_TIME < 0.1:
        return
    _LAST_PANEL_CHECK_TIME = now
    dll = _load_native()
    if dll is None:
        return
    layer = _selected_layer()
...
class _LayerPanelRefreshFilter(QtCore.QObject):
    def __init__(self):
        super().__init__()
        self._pending = False
    ...
    def eventFilter(self, obj, event):
        try:
            if event.type() not in (
                ...
                return False
            if not _inside_layers_panel(obj):
                return False
...
def start_plugin():
    ...
    _LAYER_PANEL_FILTER = _LayerPanelRefreshFilter()
    app.installEventFilter(_LAYER_PANEL_FILTER)
...
def close_plugin():
    global _LAYER_PANEL_FILTER
    _align_stop()
    if _LAYER_PANEL_FILTER is not None:
        app = QtWidgets.QApplication.instance()
        if _safe(app):
            try:
                app.removeEventFilter(_LAYER_PANEL_FILTER)
            except Exception:
                pass
        _LAYER_PANEL_FILTER = None
    for event_cls, callback in (
