# -*- coding: utf-8 -*-
"""Read-only Qt control type sniffer for Substance 3D Painter."""
...
REPORT_DIR = os.path.join(
    os.path.expanduser("~"), "Desktop", "sp_widget_sniffer_reports"
)
TEXT_PROPERTIES = (
    "text",
    "title",
...
def _write_reports():
    ...
    json_path = os.path.join(REPORT_DIR, f"sp_widget_types_{timestamp}.json")
    text_path = os.path.join(REPORT_DIR, f"sp_widget_types_{timestamp}.txt")
    records = sorted(
        _samples.values(),
        key=lambda item: (
            item.get("cpp_class", ""),
    ...
    payload = {
        "application": "Adobe Substance 3D Painter",
    ...
    print("[SP 控件嗅探器] 报告已生成：", json_path)
...
def _start():
    ...
    _timer.start()
    print("[SP 控件嗅探器] 已开始；请依次悬停、展开和点击需要分析的控件。")
...
def start_plugin():
    ...
    _menu = main_window.menuBar().addMenu("SP 控件嗅探器")
    _menu.setObjectName("sp_widget_type_sniffer_menu")
...
def close_plugin():
    ...
    if _valid(_menu):
        try:
            main_window = substance_painter.ui.get_main_window()
            main_window.menuBar().removeAction(_menu.menuAction())
        except Exception:
            pass
        _menu.deleteLater()
    _menu = None
